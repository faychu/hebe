 ...
 *  Method: hebe method with pairwise comparison
...
int num_hebe_threads = 1, num_word2vec_threads = 0, dim = 200;
...
inline void UpdateHebe(int relation, long *sampled_node_ids, int target_node_type,
...
void *TrainHebeThread(void *id) {
  long count = 0, last_count = 0;
  long limit = total_samples / num_hebe_threads + 2;
  ...
  while (1)
  {
    ...
    for (int i = 0; i < mini_batch; ++i) {
      ...
      UpdateHebe(m, sampled_node_ids, target_node_type, neg_node_offset,
...
void TrainHebe() {
  ...
  printf("Word2Vec: %s      Thread Ratio: %.2f\n",
      word2vec? "Yes": "No", (real) num_word2vec_threads / num_hebe_threads);
  ...
  start = clock();
  pthread_t *pt = 
    (pthread_t *)malloc((num_hebe_threads + num_word2vec_threads) * sizeof(pthread_t));

  for (long a = 0; a < num_hebe_threads; ++a) {
    pthread_create(&pt[a], NULL, TrainHebeThread, (void *)a);
  }
  for (long a = 0; a < num_word2vec_threads; ++a) {
    pthread_create(&pt[a + num_hebe_threads], NULL,
        TrainWord2VecThread, (void *) (a + num_hebe_threads));
  }

  for (int a = 0; a < num_hebe_threads; ++a) pthread_join(pt[a], NULL);
  for (int a = 0; a < num_word2vec_threads; ++a) pthread_cancel(pt[a + num_hebe_threads]);
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    printf("\t HEBE: HyperEdge Based Embedding\n");
    printf("Options:"); 
    ...
    printf("\t-tthread <int>\n");
    printf("\t\tUse <int> threads for hebe updating(default 1)\n");
    printf("\t-wthread <int>\n");
  ...
  if ((i = ArgPos((char *) "-tthread", argc, argv)) > 0) {
    num_hebe_threads = atoi(argv[i + 1]);
  }
